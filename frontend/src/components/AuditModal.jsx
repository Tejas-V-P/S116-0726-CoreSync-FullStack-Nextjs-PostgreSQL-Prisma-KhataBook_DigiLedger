  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/transactions/${transactionId}/audit`)
      ...
      .then((data) => {
        if (data.success) {
          setLogs(data.data || []);
          setPartyName(data.partyName || null);
          setRelatedCount(data.relatedTransactionsCount || 1);
        } else {
          ...
          <div>
            ...
            {partyName && (
              <p className="text-xs text-indigo-600 dark:text-indigo-400 flex items-center gap-1 mt-1 font-semibold">
                <User className="w-3.5 h-3.5" /> Person / Org: <span className="underline">{partyName}</span> ({relatedCount} transaction{relatedCount > 1 ? 's' : ''})
              </p>
            )}
          </div>
